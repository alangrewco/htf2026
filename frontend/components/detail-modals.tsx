...
import {
    Package, Truck, Users, CalendarDays, DollarSign, Clock, MapPin,
    Shield, Phone, Mail, FileText, CheckCircle2, AlertTriangle,
    Circle, Award, Boxes, Weight, Container,
} from "lucide-react";
import {
...
import type { Sku as ApiSku, Shipment as ApiShipment, Supplier as ApiSupplier } from "@/sdk/model";
import {
    skuDetailById, shipmentDetailById, supplierDetailById,
    type SKUDetail, type ShipmentDetail, type SupplierDetail,
    skuMetricsById, shipmentDetailsByCode, supplierMetricsById
...
export function SKUDetailModal({
    ...
    // @ts-ignore
    const riskLevel: "critical" | "high" | "medium" | "low" = metrics?.riskLevel ?? "medium";
    const conf = riskConfig[riskLevel];
    const category = metrics?.category ?? sku.description?.split("-")[0]?.trim() ?? "General";
                    ...
                    <div className="p-6 space-y-6">
                        ...
                        <DialogHeader className="space-y-3">
                            <div className="flex items-start gap-4">
                                ...
                                <Badge variant="outline" className={`${conf.bg} ${conf.color} ${conf.border} shrink-0`}>{riskLevel} risk</Badge>
                            </div>
                        </DialogHeader>
                        ...
                        <div className={`rounded-lg p-3 ${conf.bg} border ${conf.border}`}>
                            <div className="flex items-center justify-between mb-2">
                                ...
                                <span className={`text-sm font-bold ${conf.color}`}>{riskScore}/100</span>
                            </div>
                            <div className="h-2 w-full rounded-full bg-muted/50 overflow-hidden">
                                <div
                                    className={`h-full rounded-full transition-all duration-500 ${riskScore >= 70 ? "bg-urgency-critical" : riskScore >= 40 ? "bg-urgency-warning" : "bg-urgency-safe"}`}
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <InfoRow icon={DollarSign} label="Revenue" value={`$${(revenue / 1_000).toFixed(0)}K`} />
                            </div>
                        </div>
...
export function ShipmentDetailModal({
    ...
    const metrics = shipmentDetailsByCode[shipment.shipment_code];
    ...
    const uiStatus = statusMap[shipment.status] ?? "delivered";
    const conf = statusConfig[uiStatus];

    const origin = metrics?.origin ?? shipment.origin_port_id;
    const destination = metrics?.destination ?? shipment.destination_port_id;
    const etaLabel = metrics?.etaLabel ?? shipment.eta;
    const carrier = metrics?.carrier ?? shipment.route_id;
                    ...
                    <div className="p-6 space-y-6">
                        ...
                        <div className="rounded-lg border border-border/50 bg-muted/10 p-4">
                            ...
                            <div className="text-center mt-3">
                                ...
                                <span className="text-xs font-medium">{etaLabel}</span>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <InfoRow icon={Truck} label="Carrier" value={carrier} />
...
export function SupplierDetailModal({
    ...
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    const riskRating: "low" | "medium" | "high" = metrics?.riskRating ?? "medium";
    const conf = riskConfig[riskRating];
    const region = metrics?.region ?? supplier.country;
    const activeShipments = metrics?.activeShipments ?? 0;
    const plannedShipments = metrics?.plannedShipments ?? 0;
    const skus = metrics?.skus ?? [];
                    ...
                    <div className="p-6 space-y-6">
                        ...
                        <DialogHeader className="space-y-3">
                            <div className="flex items-start gap-4">
                                ...
                                <Badge variant="outline" className={`${conf.bg} ${conf.color} ${conf.border} shrink-0`}>{riskRating} risk</Badge>
                            </div>
                        </DialogHeader>
                                ...
                                <div className="flex flex-col items-center justify-center gap-1.5 rounded-lg border border-border/50 bg-muted/10 p-3">
                                    <div className="flex gap-4">
                                        <div className="text-center">
                                            <div className="text-lg font-bold text-foreground">{activeShipments}</div>
                                            <div className="text-[10px] text-muted-foreground">Active</div>
                                        </div>
                                        <div className="text-center">
                                            <div className="text-lg font-bold text-foreground">{plannedShipments}</div>
                        ...
                        {detail && (
                            <div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <InfoRow icon={Shield} label="Risk Rating" value={`${riskRating} risk`} />
                                </div>
                            </div>
                        )}
