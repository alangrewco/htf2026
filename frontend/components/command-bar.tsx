...
import { AddSKUModal, ERPImportModal } from "@/components/add-sku-modal";
  ...
  return (
    <>
      <div className="flex items-center gap-2 w-full px-4 py-2 glass-strong rounded-xl">
        ...
        <Button size="sm" className="h-8 text-xs gap-1.5" onClick={() => setAddSKUOpen(true)}>
          <Plus className="h-3.5 w-3.5" />
          Add SKU
        </Button>
        <Button variant="outline" size="sm" className="h-8 text-xs gap-1.5" onClick={() => setERPImportOpen(true)}>
      ...
      <AddSKUModal open={addSKUOpen} onOpenChange={setAddSKUOpen} />
      <ERPImportModal open={erpImportOpen} onOpenChange={setERPImportOpen} />
    </>
  );
}
