...
**New (added post-original):**

- `SECRET_KEY` — Flask session signing key (auto-generated if not set)
- `FLASK_ENV` — Set to `production` to disable debug, Swagger UI, and enable HSTS
- `CORS_ORIGINS` — Comma-separated allowed origins (default: `http://localhost:*`)
- `RATELIMIT_STORAGE_URI` — Rate limit backend (default: `memory://`, set to `redis://` for prod)
- `HOST` — Bind address (default: `127.0.0.1`)
- `MAX_CONTENT_LENGTH` — Max request body in bytes (default: `1048576` = 1 MB)

## New Features (Ported)

### Disruption Pipeline (from HTF_Hackathonb)

Full disruption → exposure → strategy → recommendation pipeline:

```bash
# Create a disruption
curl -X POST http://localhost:5001/api/disruptions \
  -H 'Content-Type: application/json' \
  -d '{"port_code":"CNSHA","disruption_type":"labor_strike","expected_delay_days":14}'

# Financial exposure analysis
curl http://localhost:5001/api/disruptions/1/exposure

# Strategy comparison
curl http://localhost:5001/api/disruptions/1/strategies

# AI recommendation with LLM drafts
curl http://localhost:5001/api/disruptions/1/recommendation

# Aggregated dashboard
curl http://localhost:5001/api/dashboard
```

### Live Signals (from WorldMonitor)

RSS-based maritime news proxy with keyword filtering:
...
# Filter by keywords
curl 'http://localhost:5001/api/signals?keywords=strike,congestion&limit=10'

# LLM-based news classification
curl -X POST http://localhost:5001/api/signals/detect \
  -H 'Content-Type: application/json' \
  -d '{"headline":"Major port strike at Shanghai disrupts trade"}'
```

### Visual Dashboard

Open http://localhost:5001/ for a self-contained visual dashboard with:
- KPI cards (revenue at risk, disruptions, affected SKUs, SLA targets)
- Tabbed disruption/exposure/strategy/recommendation views
- Live signals sidebar with keyword monitoring ("Risk Radar")

## Security

All endpoints are rate-limited and protected with security headers. See `extensions.py` and `app.py` for configuration.

| Safeguard | Implementation |
|---|---|
| Rate limiting | flask-limiter, per-route (LLM: 3-5/min, reads: 30-60/min) |
| CORS | flask-cors, localhost-only default |
| Request size | MAX_CONTENT_LENGTH = 1 MB |
| SSE timeouts | 30 min (events), 10 min (research) |
| Error sanitization | Global handlers, no stack trace leaking |
| Security headers | X-Content-Type-Options, X-Frame-Options, X-XSS-Protection, HSTS |
| Input validation | Marshmallow schemas + LLM input caps |
