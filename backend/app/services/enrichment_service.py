...
def _to_nonempty_str(value, fallback: str) -> str:
...
class EnrichmentService:
    ...
    def _persist_success(self, job_id: str, article_id: str, enrichment: dict):
        with session_scope() as session:
            ...
            metadata = enrichment.get("article_metadata") if isinstance(enrichment.get("article_metadata"), dict) else {}
            fallback_title = article.title or article.headline
            fallback_summary = enrichment.get("explanation") or article.summary or article.headline
            fallback_preview = article.preview_text or (article.body or article.headline)[:500]
            fallback_analysis = enrichment.get("explanation") or article.analysis or "Generated by enrichment agent."

            article.title = _to_nonempty_str(metadata.get("title"), fallback_title)
            article.summary = _to_nonempty_str(metadata.get("summary"), fallback_summary)
            article.preview_text = _to_nonempty_str(metadata.get("preview_text"), fallback_preview)
            article.analysis = _to_nonempty_str(metadata.get("analysis"), fallback_analysis)
            article.source_name = _to_nonempty_str(metadata.get("source_name"), article.source_name or article.source)

            keywords = metadata.get("keywords")
            if isinstance(keywords, list):
                article.keywords_json = [str(x).strip() for x in keywords if str(x).strip()]
            elif not article.keywords_json:
                article.keywords_json = list(dict.fromkeys(enrichment.get("relevance_tags") or []))

            tags = metadata.get("tags")
            if isinstance(tags, list):
                article.tags_json = [str(x).strip() for x in tags if str(x).strip()]
            elif not article.tags_json:
                article.tags_json = list(dict.fromkeys(enrichment.get("relevance_tags") or []))
            ...
            elif article.publish_datetime is None:
                article.publish_datetime = article.published_at

            article.preview_image_url = metadata.get("preview_image_url") if metadata.get("preview_image_url") else None
